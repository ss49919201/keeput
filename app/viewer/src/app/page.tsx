  ...
  return (
    <div className="font-sans min-h-screen p-8 sm:p-12">
      ...
      {error ? (
        ...
        <div className="rounded border border-black/10 dark:border-white/15">
          <div className="grid grid-cols-[200px_1fr] gap-0 text-sm">
            <div className="px-3 py-2 font-medium bg-black/[.04] dark:bg-white/[.06]">
              Key
            </div>
            <div className="px-3 py-2 font-medium bg-black/[.04] dark:bg-white/[.06]">
              Value
            ...
            {items.map((it) => (
              <div className="contents" key={it.key}>
                <div className="px-3 py-2 border-t border-black/5 dark:border-white/10 font-mono">
                  {it.key}
                </div>
                <div className="px-3 py-2 border-t border-black/5 dark:border-white/10 font-mono break-all">
                  {it.value}
                </div>
              </div>
            ))}
          ...
          {items.length === 0 && (
            <div className="px-3 py-4 text-sm text-black/70 dark:text-white/70">
              データが存在しません
            </div>
          )}
        </div>
      ...
      <p className="mt-4 text-xs text-black/60 dark:text-white/60">
        このページは prefix `dummy:` のキーを読み込みます。ダミーデータの投入は
        npm script を使用してください。
      </p>
    </div>
  );
}
